...
def graph_to_mask_pixel(S, cc, image_shape):
    """
    Version adaptée de graph_to_mask pour les caractéristiques basées sur les pixels
    """
    # Reshape clustered graph en fonction de la forme originale de l'image
    h, w = image_shape[:2]  # hauteur et largeur de l'image
    S = np.array(torch.reshape(S, (h, w)))
    ...
    # S est déjà à la taille de l'image, pas besoin de redimensionner
    mask = S.astype('float')
...
def load_data_sparse(adj_sparse, node_feats):
    """
    Load data to pytorch-geometric data format when adj is a sparse matrix
    @param adj_sparse: Sparse adjacency matrix of a graph
    @param node_feats: Feature matrix of a graph
    @return: Graph in pytorch-geometric data format and sparse adjacency in PyTorch format
    """
    ...
    return node_feats, edge_index, edge_weight, sparse_tensor
